        ...
        >
          ...
          <el-table-column prop="status" label="状态">
            <template slot-scope="scope">
              <el-tag v-if="scope.row.status === 0" type="success">修改成功</el-tag>
              <el-tag v-else-if="scope.row.status === 1" type="danger">原密码不正确</el-tag>
              <el-tag v-else-if="scope.row.status === 2" type="info">教务员重置</el-tag>
...
<script>
import { getUpdatePasswordRecords } from '@/api/user'
...
export default {
  ...
  methods: {
    async refreshRecords() {
      try {
        const { data: response } = await getUpdatePasswordRecords(
